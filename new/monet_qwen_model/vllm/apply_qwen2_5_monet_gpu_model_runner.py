...
print("[PATCH] apply_qwen2_5_monet_gpu_model_runner loaded:", __file__)

patch_path = pathlib.Path(__file__).with_name("monet_gpu_model_runner.py")

# Load the patched module once under a private name
spec = importlib.util.spec_from_file_location("monet_gpu_model_runner_patched", patch_path)
...
for key in CANDIDATE_KEYS:
    if key in sys.modules:
        print(f"[PATCH] apply_qwen2_5_monet_gpu_model_runner: patching {key}")
        del sys.modules[key]
