...
class Qwen2_5_VLCausalLMOutputAVT(ModelOutput):
    ...
    teacher_attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward_latent_embeds(
        ...
        ###################################################
        # WARNING: ONLY SUPPORT BSZ=1 !
        ###################################################
        
        all_hidden_states = []
        all_hidden_states_all_layers = None
        #if self.config.stage == 'stage2' and not generate_mode:
        if self.config.stage == 'avt_stage1' and not generate_mode:
            ...
            for latent_i in range(max_n_latents):
                ...
                next_compute_range = (
                    ...
                    (
                        input_ids.shape[1]
                        if latent_i + 1 >= max_n_latents
                        else latent_lists[0][latent_i + 1]
                    ),
                )
                ...
                if all_hidden_states_all_layers is None:
                    all_hidden_states_all_layers = outputs.hidden_states
                else:
            ...
            batch_size, seq_length, _ = curr_inputs_embeds.shape
            delta = (
                (cache_position[0] + self.rope_deltas).cpu().to(inputs_embeds.device)
                if cache_position is not None
                else 0
            )
            position_ids = torch.arange(seq_length, device=inputs_embeds.device)
