...
def avt_single_input_images_preprocess_function(sample, dataset_root="", processor=None, max_seq_len=4096, cur_max=-1, id=0, rank=-1):
    ...
    texts = [processor.apply_chat_template(sample, tokenize=False)]
    ...
    if batch["input_ids"].shape[1] > max_seq_len:
        print(dataset_name, batch["input_ids"].shape[1])
        return None, cur_max
...
def filter_invalid_samples_in_json(json_path, dataset_root, processor, cur_max=-1, id=-1, max_seq_len=4096):
    ...
    for idx, sample in tqdm(enumerate(data), desc=json_path, total=len(data)):
        result, cur_max = avt_single_input_images_preprocess_function(
            sample, dataset_root=dataset_root, processor=processor, rank=0, max_seq_len=max_seq_len, cur_max=cur_max, id=id
        )
